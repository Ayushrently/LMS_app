...
RSpec.describe User, type: :model do
  ...
  describe ".includes ransackable attributes" do
    it("includes only the specified attributes") do
      expect(User.ransackable_attributes).to match_array(["id", "email", "created_at", "updated_at"])
    end
  end

  describe ".includes ransackable associations" do
    it("includes only the specified associations") do
      expect(User.ransackable_associations).to match_array(["profile", "comments", "enrollments", "courses", "authored_courses", "subscription"])
    end
  end
