class Api::V1::CommentsController < Api::V1::BaseController
  ...
  before_action :ensure_author_for_comment!, only: %i[update destroy]

  def show
    render json: @comment.as_json(only: %i[id body user_id commentable_type commentable_id])
  end
  ...
  def set_commentable
    @commentable = if params[:lesson_id]
                     @course.lessons.find_by(id: params[:lesson_id])
                   else
                     @course
                   end
