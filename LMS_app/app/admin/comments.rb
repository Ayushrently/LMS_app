ActiveAdmin.register Comment, as: "UserComment" do
  ...
  filter :commentable_type, as: :select, collection: ["Course", "Lesson"]
  ...
  form do |f|
    f.inputs "Comment Details" do
      f.input :user, as: :select, collection: User.joins(:profile).map { |u| ["#{u.profile.username} (#{u.email})", u.id] }
      f.input :body
      f.input :commentable_type, as: :select, collection: ["Course", "Lesson"]
